...
## Anleitung für User
settings_template.toml auf Mikrocontroller kopieren und in settings.toml umbenennen.
boot.py und code.py auf Mikrocontroller kopieren.
Über Serielle Schnittstelle ist es möglich, die Outputs/logs zu lesen.
WLAN-SSID und Passwort in settings.toml einrichten.
Andere Einstellungen, wie z.B. MQTT IP-Addresse, username und password können auch eingestellt werden.
Mikrokontroller verbindet sich mit Wlan und fängt an Daten an MQTT-Broker zu senden.
...
#### 4. MQTT
Die Sensordaten werden im Intervall (reading_interval_seconds) an den Broker gesendet. Statusmeldungen ("online", "offline") werden über das Status-Topic veröffentlicht.

#### 4. Webserver verwenden:

Als Standardport fürden Webserver wird port `8080` verwendet.
